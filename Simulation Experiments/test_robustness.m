function [ kPL_error_opt, kPL_error_const, kPL_error_RF_compensated, parameters_of_interest_est_opt, parameters_of_interest_est_const, parameters_of_interest_est_RF_compensated ] = test_robustness(model, parameter_to_vary, parameter_values, num_trials, thetas_opt, thetas_const, thetas_RF_compensated, kTRANS_val, kPL_val, R1P_val, R1L_val, u_est, sigma_squared )
...
%   assuming that parameter_values(i) is the true value of parameter
    ...
    syms R1P R1L kPL kTRANS input_scale
    ...
    % for param_count = 1:length(parameter_values) 
        ...
        joint_param_est(:, param_count) = fmincon(obj, var_initial, [], [], [], [], [0 0 0], [1 1 1], [], options);
    ...
    % for param_count = 1:length(parameter_values)   
        ...
        for trial=1:num_trials
            ...
            obj = @(var) negative_log_likelihood_rician(var, [], sigma_squared, thetas_opt, y_thetas_opt(:, :, trial, param_count), u_est_opt(:, param_count)); 
            ...
            obj = @(var) negative_log_likelihood_rician(var, [], sigma_squared, thetas_const, y_thetas_const(:, :, trial, param_count), u_est_const(:, param_count)); 
            options = optimset( 'Display', 'iter', 'MaxFunEvals', 1000); 
            parameters_of_interest_est_const(trial, :, param_count) = fminunc(obj, var_initial, options);

            obj = @(var) negative_log_likelihood_rician(var, [], sigma_squared, thetas_RF_compensated, y_thetas_RF_compensated(:, :, trial, param_count), u_est_const(:, param_count)); 
            options = optimset( 'Display', 'iter', 'MaxFunEvals', 1000); 
